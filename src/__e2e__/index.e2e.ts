...
describe('e2e tests', () => {
  it('should complete provided jobs and return processed jobs', async () => {
    const processor = BatchRunner.create<string>({
      batchSize: 2,
      concurrency: 2,
    ...
    for (let i = 1; i <= 20; i++) {
      if (i % 3 === 0) {
        processor.addJob(createTimedFailedJob(500, String(i)).data as Job<string>);
      } else {
        processor.addJob(createTimedJob(500, String(i)).data as Job<string>);
      }
    }
    ...
    processor.start();
    const status1 = processor.getBatchRunnerStatus();
    ...
    await wait(5000);
    const status2 = processor.getBatchRunnerStatus();
